...
def lambda_handler():
    ...
    # Status check
    # status_export_file_query = "Select * from api_export where file_type='EmployeesDocuments' and status='Generating'"
    # cur_users.execute(status_export_file_query)
    # status_exports = cur_users.fetchall()
    # if len(status_exports) > 0:
    #     return {'status1': False}

    export_file_query = "Select * from api_export where file_type='EmployeesDocuments' and exfile_iscreated = FALSE LIMIT 1"
    ...
    for export in exports:
        # try:
            query_employees = 'SELECT emp_id,name,pan_number,pan_card,address_proof,permanent_proof,aadharcard_number,aadharcard ' \
                              'FROM api_employee'
